/*
 * Lab 06 — WCET Measurement with DWT Cycle Counter (WeAct STM32H503CBT6)
 *
 * Uses the Cortex-M33 DWT (Data Watchpoint and Trace) unit's CYCCNT register
 * to measure execution time of various workloads in CPU cycles.
 * At 250 MHz, 1 cycle = 4 ns.
 */
...
int main(void)
{
    ...
    printf("\r\n=== RTOS Lab 06: WCET & DWT Measurement ===\r\n");
    printf("CPU: 250 MHz  |  DWT CYCCNT: 1 cycle = 4 ns\r\n\r\n");

    xUartMutex = xSemaphoreCreateMutex();
    configASSERT(xUartMutex != NULL);

    xTaskCreate(vWcetTask,    "WCET",    512, NULL, 2, NULL);
    xTaskCreate(vLatencyTask, "Latency", 256, NULL, 5, NULL);
