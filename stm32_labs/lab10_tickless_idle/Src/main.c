/*
 * Lab 10 — Tickless Idle / Low-Power RTOS (WeAct STM32H503CBT6)
 *
 * configUSE_TICKLESS_IDLE = 1:
 *   The FreeRTOS ARM CM33 port suppresses SysTick when all tasks are blocked.
 *   The CPU executes WFI (Wait For Interrupt), halting the pipeline until the
 *   next interrupt (next task wakeup or peripheral event) fires.
 *
 * Demonstration:
 *   - vLedTask     blinks PC13 every 500 ms (system alive indicator)
 *   - vSensorTask  wakes every 2 s, does ~1 ms of work, then blocks again
 *   - vStatsTask   prints idle-entry count and tick-ISR count every 5 s
 *   - vApplicationIdleHook  increments ulIdleCount each idle entry
 *   - vApplicationTickHook  increments ulTickCount each tick that fires
 *     (tickless idle suppresses most ticks, so this count stays low)
 */
...
int main(void)
{
    ...
    printf("configUSE_TICKLESS_IDLE = 1  (WFI sleep between tasks)\r\n");
#else
    printf("configUSE_TICKLESS_IDLE = 0  (busy-spin idle, for comparison)\r\n");
#endif
    printf("Tasks: LED=500ms  Sensor=2000ms  Stats=5000ms\r\n\r\n");

    xUartMutex = xSemaphoreCreateMutex();
    configASSERT(xUartMutex != NULL);

    xTaskCreate(vLedTask,    "LED",    384, NULL, 2, NULL);
...
void vApplicationIdleHook(void)
{
     ...
     * With tickless idle: called once before WFI, then again after wakeup.
     * Without tickless idle: called in a tight loop — ulIdleCount rockets.
     */
    ulIdleCount++;

    /*
     * HAL_PWR_EnterSLEEPMode is NOT called here — the FreeRTOS port's
     * vPortSuppressTicksAndSleep() handles WFI internally when tickless
     * idle is enabled.  Calling WFI here would interfere with tick accounting.
     */
}

void vApplicationTickHook(void)
{
    HAL_IncTick();   /* keep HAL time base alive */
    ulTickCount++;   /* count actual SysTick ISR fires (suppressed when sleeping) */
}
