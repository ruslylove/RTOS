...
\begin{warnbox}
  \textbf{Timer task hook also required.}
  When \texttt{configSUPPORT\_STATIC\_ALLOCATION = 1} \emph{and}
  \texttt{configUSE\_TIMERS = 1} (the default in this lab's
  \file{FreeRTOSConfig.h}), the FreeRTOS timer task also needs static memory.
  You must define \textbf{both} hooks or the linker will fail with
  \texttt{undefined reference to `vApplicationGetTimerTaskMemory'}:
...
\end{lstlisting}
  ...
  Add this alongside \texttt{vApplicationGetIdleTaskMemory} in \file{static\_alloc.c}.
\end{warnbox}
