...
\subsection{GPIO Interrupt Flag API (SDK v2.x)}

The current NXP MCUXpresso SDK uses a per-pin API to clear GPIO interrupt flags.
Use \api{GPIO\_PinClearInterruptFlag} instead of the old mask-based function:

\begin{lstlisting}[style=cstyle]
/* Correct -- current SDK (MCUXpresso SDK 2.x) */
GPIO_PinClearInterruptFlag(GPIO0, 23u);   /* pin number, not bit mask */

/* Old API -- removed from current SDK, will fail to link */
/* GPIO_ClearPinsInterruptFlags(GPIO0, 1u << 23); */
\end{lstlisting}

The function prototype is declared in \file{fsl\_gpio.h}:
\begin{lstlisting}[style=cstyle]
