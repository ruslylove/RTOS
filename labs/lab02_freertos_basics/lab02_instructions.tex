...
\begin{tabular}{@{}p{3.8cm} p{10cm}@{}}
  ...
  \textbf{Toolchain}      & ARM GNU Toolchain 14.2 (\texttt{arm-none-eabi-gcc}) + Make \\[2pt]
  \textbf{Flash / Debug}  & pyocd 0.44 via local \texttt{.venv} (target \texttt{mcxn236vdf}) \\[2pt]
...
\begin{checklist}
  \item ARM GNU Toolchain 14.2 installed; \texttt{make} succeeds in
        \file{lab01\_setup\_verify/}
  \item NXP MCUXpresso SDK at the path set in \texttt{SDK\_ROOT} in \file{Makefile}
  \item \textbf{pyocd} available: \texttt{C:/Development/RTOS/.venv/Scripts/pyocd.exe}
        (local venv; \texttt{NXP.MCXN236\_DFP} pack pre-installed)
...
The progress bar completes and the board starts running immediately after the reset.
