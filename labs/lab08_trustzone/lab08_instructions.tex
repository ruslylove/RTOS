...
\begin{warnbox}
  \textbf{Flash order matters.} The Secure image configures the SAU and releases
  the NS core. Always flash \file{secure/} before \file{nonsecure/} or the NS
  application will start without a valid SAU configuration.
\end{warnbox}
...
\subsection{Board Hardware Init Declaration (\texttt{app.h})}

\api{BOARD\_InitHardware()} is declared in \file{app.h} (from the SDK's
\file{freertos\_hello} example), \emph{not} in \file{hardware\_init.h}.
Both the secure and non-secure \file{main} files must include it:

\begin{lstlisting}[style=cstyle]
#include "board.h"
#include "app.h"      /* declares BOARD_InitHardware() */
\end{lstlisting}

Without this include the compiler emits:
\texttt{error: implicit declaration of function `BOARD\_InitHardware'}.
